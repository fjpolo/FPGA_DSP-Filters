...
class FixedPointAdderDriver(uvm_driver):
...
class FixedPointAdderMonitor(uvm_monitor):
...
class FixedPointAdderAgent(uvm_agent):
    def build_phase(self):
        self.monitor = FixedPointAdderMonitor.create("monitor", self)
        self.driver = FixedPointAdderDriver.create("driver", self)
        self.sequencer = uvm_sequencer("sequencer", self)
        
    def connect_phase(self):
        self.driver.seq_item_port.connect(self.sequencer.seq_item_export)

class FixedPointAdderScoreboard(uvm_scoreboard):
    def build_phase(self):
...
class FixedPointAdderEnv(uvm_env):
    def build_phase(self):
        self.agent = FixedPointAdderAgent.create("agent", self)
        self.scoreboard = FixedPointAdderScoreboard.create("scoreboard", self)
...
class FixedPointAdderTest(uvm_test):
    def build_phase(self):
        self.env = FixedPointAdderEnv.create("env", self)
...
async def run_test(dut):
    await uvm_root().run_test("FixedPointAdderTest")