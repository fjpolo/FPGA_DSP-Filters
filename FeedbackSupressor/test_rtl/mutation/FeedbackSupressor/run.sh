    ...
    cp ${PWD}/../../../rtl/FeedbackSupressor.v .

    # Append `define MCY after `timescale 1ps/1ps to FeedbackSupressor.v using awk
    awk '1;/`timescale 1ps\/1ps/{print "`define MCY"}' FeedbackSupressor.v > template_temp.v
    mv template_temp.v FeedbackSupressor.v

    #replace the orginal testbench file with the temp file.
    mv template_temp.v FeedbackSupressor.v

    # Copy FeedbackSupressor here
    cp ${PWD}/../../simulation/icarus/FeedbackSupressor/testbench.v .
