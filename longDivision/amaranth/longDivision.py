...
# Main module #
###############
class longDivision(wiring.Component):
    """
    ...
    def formal(cls) -> Tuple[Module, List[Signal]]:
        """Formal verification for the longDivision module."""
        m = Module()
        # Instantiate your module with a specific limit for formal verification
        m.submodules.longDivision = longDivision = cls(limit=16)

        # --- Formal Properties ---

        m.d.comb += Cover(longDivision.count == 0)
        m.d.comb += Cover(longDivision.count == longDivision.limit)
        m.d.comb += Cover(longDivision.ovf == 1)

        # Return the module 'm' (which contains 'c' as a submodule)
        # and the ports of your counter module 'c'.
        return m, [longDivision.en, longDivision.count, longDivision.ovf]
...
with sim.write_vcd("longDivision.vcd"):
    sim.run()
...
# --- CONVERT ---
top = longDivision(25)
with open("longDivision.v", "w") as f:
