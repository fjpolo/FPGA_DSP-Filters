    ...
    cp ${PWD}/../../../rtl/longDivision.v .

    # Append `define MCY after `timescale 1ps/1ps to longDivision.v using awk
    awk '1;/`timescale 1ps\/1ps/{print "`define MCY"}' longDivision.v > template_temp.v
    mv template_temp.v longDivision.v

    #replace the orginal testbench file with the temp file.
    mv template_temp.v longDivision.v

    # Copy longDivision here
    cp ${PWD}/../../simulation/icarus/longDivision/testbench.v .
