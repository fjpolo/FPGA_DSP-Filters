    ...
    # Append `define MCY after `timescale 1ps/1ps to pll.v using awk
    awk '1;/`timescale 1ps\/1ps/{print "`define MCY"}' pll.v > template_temp.v
    mv template_temp.v pll.v

    #replace the orginal testbench file with the temp file.
    mv template_temp.v pll.v

    # Copy pll here
    cp ${PWD}/../../simulation/icarus/pll/testbench.v .
