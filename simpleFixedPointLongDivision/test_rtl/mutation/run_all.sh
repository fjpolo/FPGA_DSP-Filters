...
for dir in */; do
  ...
  if [ -f "$dir/run.sh" ]; then
    ...
    (cd "$dir" && ./run.sh >> simpleFixedPointLongDivision_log.txt)
    exit_status=$?

    # Check for EQGAP and FMONLY
    if grep -q "mutations as" ${PWD}/${dir}/simpleFixedPointLongDivision_log.txt; then
    ...
    if [ $exit_status -ne 0 ]; then
      echo "    [MUTATION] FAIL: simpleFixedPointLongDivision failed!"
    else
      echo "    [MUTATION] PASS: simpleFixedPointLongDivision passed!"
    fi
  else
