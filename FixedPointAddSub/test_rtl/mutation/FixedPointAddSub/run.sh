    ...
    cp ${PWD}/../../../rtl/FixedPointAddSub.v .

    # Append `define MCY after `timescale 1ps/1ps to FixedPointAddSub.v using awk
    awk '1;/`timescale 1ps\/1ps/{print "`define MCY"}' FixedPointAddSub.v > template_temp.v
    mv template_temp.v FixedPointAddSub.v

    #replace the orginal testbench file with the temp file.
    mv template_temp.v FixedPointAddSub.v

    # Copy FixedPointAddSub here
    cp ${PWD}/../../simulation/icarus/FixedPointAddSub/testbench.v .
