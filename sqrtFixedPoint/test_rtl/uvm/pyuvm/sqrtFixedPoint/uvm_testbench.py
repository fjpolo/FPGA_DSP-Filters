...
class sqrtFixedPointDriver(uvm_driver):
...
class sqrtFixedPointMonitor(uvm_monitor):
...
class sqrtFixedPointAgent(uvm_agent):
    def build_phase(self):
        self.monitor = sqrtFixedPointMonitor.create("monitor", self)
        self.driver = sqrtFixedPointDriver.create("driver", self)
        self.sequencer = uvm_sequencer("sequencer", self)
        
    def connect_phase(self):
        self.driver.seq_item_port.connect(self.sequencer.seq_item_export)

class sqrtFixedPointScoreboard(uvm_scoreboard):
    def build_phase(self):
...
class sqrtFixedPointEnv(uvm_env):
    def build_phase(self):
        self.agent = sqrtFixedPointAgent.create("agent", self)
        self.scoreboard = sqrtFixedPointScoreboard.create("scoreboard", self)
...
class sqrtFixedPointTest(uvm_test):
    def build_phase(self):
        self.env = sqrtFixedPointEnv.create("env", self)
...
async def run_test(dut):
    await uvm_root().run_test("sqrtFixedPointTest")