...
class hsFIRcheapDriver(uvm_driver):
...
class hsFIRcheapMonitor(uvm_monitor):
...
class hsFIRcheapAgent(uvm_agent):
    def build_phase(self):
        self.monitor = hsFIRcheapMonitor.create("monitor", self)
        self.driver = hsFIRcheapDriver.create("driver", self)
        self.sequencer = uvm_sequencer("sequencer", self)
        
    def connect_phase(self):
        self.driver.seq_item_port.connect(self.sequencer.seq_item_export)

class hsFIRcheapScoreboard(uvm_scoreboard):
    def build_phase(self):
...
class hsFIRcheapEnv(uvm_env):
    def build_phase(self):
        self.agent = hsFIRcheapAgent.create("agent", self)
        self.scoreboard = hsFIRcheapScoreboard.create("scoreboard", self)
...
class hsFIRcheapTest(uvm_test):
    def build_phase(self):
        self.env = hsFIRcheapEnv.create("env", self)
...
async def run_test(dut):
    await uvm_root().run_test("hsFIRcheapTest")