    ...
    # Append `define MCY after `timescale 1ps/1ps to gain.v using awk
    awk '1;/`timescale 1ps\/1ps/{print "`define MCY"}' gain.v > template_temp.v
    mv template_temp.v gain.v

    #replace the orginal testbench file with the temp file.
    mv template_temp.v gain.v

    # Copy gain here
    cp ${PWD}/../../simulation/icarus/gain/testbench.v .
