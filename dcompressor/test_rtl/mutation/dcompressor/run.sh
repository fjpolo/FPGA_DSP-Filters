    ...
    cp ${PWD}/../../../rtl/dcompressor.v .

    # Append `define MCY after `timescale 1ps/1ps to dcompressor.v using awk
    awk '1;/`timescale 1ps\/1ps/{print "`define MCY"}' dcompressor.v > template_temp.v
    mv template_temp.v dcompressor.v

    #replace the orginal testbench file with the temp file.
    mv template_temp.v dcompressor.v

    # Copy dcompressor here
    cp ${PWD}/../../simulation/icarus/dcompressor/testbench.v .
