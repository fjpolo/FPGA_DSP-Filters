    ...
    cp ${PWD}/../../../rtl/Regfile.v .

    # Append `define MCY after `timescale 1ps/1ps to Regfile.v using awk
    awk '1;/`timescale 1ps\/1ps/{print "`define MCY"}' Regfile.v > template_temp.v
    mv template_temp.v Regfile.v

    #replace the orginal testbench file with the temp file.
    mv template_temp.v Regfile.v

    # Copy Regfile here
    cp ${PWD}/../../simulation/icarus/Regfile/testbench.v .
