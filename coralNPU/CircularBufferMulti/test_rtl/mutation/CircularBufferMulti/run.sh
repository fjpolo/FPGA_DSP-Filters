    ...
    cp ${PWD}/../../../rtl/CircularBufferMulti.v .

    # Append `define MCY after `timescale 1ps/1ps to CircularBufferMulti.v using awk
    awk '1;/`timescale 1ps\/1ps/{print "`define MCY"}' CircularBufferMulti.v > template_temp.v
    mv template_temp.v CircularBufferMulti.v

    #replace the orginal testbench file with the temp file.
    mv template_temp.v CircularBufferMulti.v

    # Copy CircularBufferMulti here
    cp ${PWD}/../../simulation/icarus/CircularBufferMulti/testbench.v .
