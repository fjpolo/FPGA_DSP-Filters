...
class hsFIRMonitor(uvm_monitor):
...
class hsFIRAgent(uvm_agent):
    def build_phase(self):
        self.monitor = hsFIRMonitor.create("monitor", self)
        self.driver = hsFIRDriver.create("driver", self)
        self.sequencer = uvm_sequencer("sequencer", self)
        
    def connect_phase(self):
        self.driver.seq_item_port.connect(self.sequencer.seq_item_export)

class hsFIRScoreboard(uvm_scoreboard):
    def build_phase(self):
...
class hsFIREnv(uvm_env):
    def build_phase(self):
        self.agent = hsFIRAgent.create("agent", self)
        self.scoreboard = hsFIRScoreboard.create("scoreboard", self)
...
class hsFIRTest(uvm_test):
    def build_phase(self):
        self.env = hsFIREnv.create("env", self)
...
async def run_test(dut):
    await uvm_root().run_test("hsFIRTest")