...
class pwmaudioDriver(uvm_driver):
...
class pwmaudioMonitor(uvm_monitor):
...
class pwmaudioAgent(uvm_agent):
    def build_phase(self):
        self.monitor = pwmaudioMonitor.create("monitor", self)
        self.driver = pwmaudioDriver.create("driver", self)
        self.sequencer = uvm_sequencer("sequencer", self)
        
    def connect_phase(self):
        self.driver.seq_item_port.connect(self.sequencer.seq_item_export)

class pwmaudioScoreboard(uvm_scoreboard):
    def build_phase(self):
...
class pwmaudioEnv(uvm_env):
    def build_phase(self):
        self.agent = pwmaudioAgent.create("agent", self)
        self.scoreboard = pwmaudioScoreboard.create("scoreboard", self)
...
class pwmaudioTest(uvm_test):
    def build_phase(self):
        self.env = pwmaudioEnv.create("env", self)
...
async def run_test(dut):
    await uvm_root().run_test("pwmaudioTest")