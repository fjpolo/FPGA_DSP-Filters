    ...
    cp ${PWD}/../../../rtl/RVDSPCoProc.v .

    # Append `define MCY after `timescale 1ps/1ps to RVDSPCoProc.v using awk
    awk '1;/`timescale 1ps\/1ps/{print "`define MCY"}' RVDSPCoProc.v > template_temp.v
    mv template_temp.v RVDSPCoProc.v

    #replace the orginal testbench file with the temp file.
    mv template_temp.v RVDSPCoProc.v

    # Copy RVDSPCoProc here
    cp ${PWD}/../../simulation/icarus/RVDSPCoProc/testbench.v .
