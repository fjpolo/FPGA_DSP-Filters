    ...
    cp ${PWD}/../../../rtl/lcompressor.v .

    # Append `define MCY after `timescale 1ps/1ps to lcompressor.v using awk
    awk '1;/`timescale 1ps\/1ps/{print "`define MCY"}' lcompressor.v > template_temp.v
    mv template_temp.v lcompressor.v

    #replace the orginal testbench file with the temp file.
    mv template_temp.v lcompressor.v

    # Copy lcompressor here
    cp ${PWD}/../../simulation/icarus/lcompressor/testbench.v .
