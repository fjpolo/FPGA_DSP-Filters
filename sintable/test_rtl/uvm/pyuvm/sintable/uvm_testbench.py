...
class sintableDriver(uvm_driver):
...
class sintableMonitor(uvm_monitor):
...
class sintableAgent(uvm_agent):
    def build_phase(self):
        self.monitor = sintableMonitor.create("monitor", self)
        self.driver = sintableDriver.create("driver", self)
        self.sequencer = uvm_sequencer("sequencer", self)
        
    def connect_phase(self):
        self.driver.seq_item_port.connect(self.sequencer.seq_item_export)

class sintableScoreboard(uvm_scoreboard):
    def build_phase(self):
...
class sintableEnv(uvm_env):
    def build_phase(self):
        self.agent = sintableAgent.create("agent", self)
        self.scoreboard = sintableScoreboard.create("scoreboard", self)
...
class sintableTest(uvm_test):
    def build_phase(self):
        self.env = sintableEnv.create("env", self)
...
async def run_test(dut):
    await uvm_root().run_test("sintableTest")